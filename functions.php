<?php
/**
* boomtennis functions and definitions
...
function boomtennis_scripts() {

    // load Bootstrap CSS v5.1
    wp_enqueue_style( 'boomtennis-bootstrap-css', THEME_DIR_URI . '/includes/css/bootstrap.min.css');

    // load website CSS, versioned
    wp_enqueue_style( 'boomtennis-css', THEME_DIR_URI . '/assets/css/style.css', [], filemtime( get_stylesheet_directory() . '/assets/css/style.css') );

	// load Bootstrap JS v5.1
	wp_enqueue_script( 'boomtennis-bootstrap-js', THEME_DIR_URI . '/includes/js/bootstrap.min.js', array('jquery') );
}
add_action( 'wp_enqueue_scripts', 'boomtennis_scripts' );
...
/* Register menus */
if ( ! function_exists( 'boomtennis_setup' ) ) :
	function boomtennis_setup() {

    register_nav_menus( array(
        'primary' => __( 'Primary Menu', 'boomtennis' ),
        'footer'  => __( 'Footer Menu', 'boomtennis' ),
    ) );
